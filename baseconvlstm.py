...
class ModifiedConvLSTMCell(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, kernel_size, stride, padding, bias):
        """
        Initialize ConvLSTM cell.
        Parameters
        ----------
    ...
    def forward(self, input_tensor, cur_state):
        ...
        combined_conv = self.conv(combined) # i gate, f gate, o gate, g gate are calculated together, and then split
        # print(f'Combined conv output shape: {combined_conv.shape}')
        cc_i, cc_f, cc_o, cc_g = torch.split(combined_conv, self.hidden_dim, dim=1)
        i = torch.sigmoid(cc_i)
        f = torch.sigmoid(cc_f)
        o = torch.sigmoid(cc_o)
        ...
        c_next = f * c_cur + i * g # c state tensor update
        h_next = o * torch.tanh(c_next) # h state tensor update
        # print(f'h_next shape: {h_next.shape}, c_next shape: {c_next.shape}')
        return h_next, c_next # Output two state tensors of the current timestamp
...
class ModifiedConvLSTM(nn.Module):
    ...
    """
    def __init__(self, input_dim, hidden_dim, kernel_size, stride, padding, num_layers,
                 batch_first=False, bias=True, return_all_layers=False):
        super(ModifiedConvLSTM, self).__init__()
 
        self._check_kernel_size_consistency(kernel_size)
 
        kernel_size = self._extend_for_multilayer(kernel_size, num_layers)
        hidden_dim = self._extend_for_multilayer(hidden_dim, num_layers)
        stride = self._extend_for_multilayer(stride, num_layers)   # New: Handle stride for each layer
        padding = self._extend_for_multilayer(padding, num_layers) # New: Handle padding for each layer
 
        if not len(kernel_size) == len(hidden_dim) == len(stride) == len(padding) == num_layers:
            raise ValueError('Inconsistent list length.')
        ...
        for i in range(0, self.num_layers):
            ...
            cell_list.append(ModifiedConvLSTMCell(input_dim=cur_input_dim,
                                          hidden_dim=self.hidden_dim[i],
                                          kernel_size=self.kernel_size[i],
                                          stride=self.stride[i],         # Pass stride to ConvLSTMCell
                                          padding=self.padding[i],       # Pass padding to ConvLSTMCell
                                          bias=self.bias))
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        for layer_idx in range(self.num_layers): # Calculate layer by layer
            ...
            # print(f'Layer {layer_idx} last state h shape: {h.shape}, c shape: {c.shape}')
        if not self.return_all_layers:
            layer_output_list = layer_output_list[-1:]
            last_state_list = last_state_list[-1:]
        return layer_output_list, last_state_list
