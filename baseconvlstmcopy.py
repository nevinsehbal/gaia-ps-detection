...
class ConvLSTMCell(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, kernel_size, bias, num_conv_layers=4):
        ...
        num_conv_layers: int
            Number of convolutional layers in each ConvLSTM cell.
        """
        ...
        self.padding = kernel_size[0] // 2, kernel_size[1] // 2  # Ensure (h, w) remains unchanged
        self.bias = bias
        self.num_conv_layers = num_conv_layers  # Number of convolutional layers

        # First convolutional layer (input + hidden state channels)
        self.convs = nn.ModuleList([nn.Conv2d(in_channels=self.input_dim + self.hidden_dim,
        ...
        for _ in range(1, num_conv_layers):
            self.convs.append(nn.Conv2d(in_channels=4 * self.hidden_dim,
    ...
    def forward(self, input_tensor, cur_state):
        ...
        # Pass through all convolutional layers sequentially
        combined_conv = self.convs[0](combined)
        for i in range(1, self.num_conv_layers):
            combined_conv = torch.relu(self.convs[i](combined_conv))  # Use ReLU between conv layers

        # Split the output into i, f, o, g gates
    ...
    def init_hidden(self, batch_size, image_size):
        ...
        init_h = torch.zeros(batch_size, self.hidden_dim, height, width, device=self.convs[0].weight.device)
        init_c = torch.zeros(batch_size, self.hidden_dim, height, width, device=self.convs[0].weight.device)
